...
class Logger(object):
    ...
    def setup_main_logger(self):
        ...
        # Add logger file
        if self.logger_file:
            fh = self.setup_logfile(self.logger_file, console_loglevel=self.console_loglevel, format=formatter)
            self._add_logfiles_to(logger, fh)

        logger.propagate = False
        if not any(isinstance(handler, type(streamhandler)) for handler in logger.handlers):
    ...
    def _add_report_level(self, logger):
        ...
        def report(self, msg, *args, **kwargs):
            if self.isEnabledFor(REPORT_LEVEL):
                self._log(REPORT_LEVEL, colored.magenta(msg), args, **kwargs)
        logger.report = types.MethodType(report, logger)
